...
Push.Configure = function(options) {
    ...
    if (options.apn) {
        ...
        apnConnection.on('transmissionError', Meteor.bindEnvironment(function (errCode, notification, recipient) {
          if (Push.debug) {
            console.log('Got error code %d for token %s', errCode, recipient.token);
          }
        ...
        var initFeedback = function() {
            ...
            feedback.on("feedback", function(devices) {
                devices.forEach(function(item) {
                    ...
                    // from the manual: Use the timestamp to verify that the device tokens haven’t
                    // been reregistered since the feedback entry was generated. For each
                    // device that has not been reregistered, stop sending notifications.

                  //TODO only remove where token matches and last reg time < feedback time
